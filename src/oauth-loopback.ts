 ...
 * (the "Desktop app" client flow). This file statically imports Node's `http`
 * and `net` modules, so it MUST NOT be imported on mobile — `oauth.ts` loads it
 * lazily via dynamic `import()` only inside the desktop branch.
 */
